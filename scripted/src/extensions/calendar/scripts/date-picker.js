...
Exhibit.DatePickerFacet.DatePicker.prototype.highlight = function(elmt) {
  if (this._highlight) {
    ...
    Exhibit.jQuery("td.day").each(function(i){
      Exhibit.jQuery('#'+this.id).removeClass('highlight');
    });
    ...
    while(center <= end) {
      Exhibit.jQuery('#'+ Exhibit.DateUtil.formatDate(center, this._facet._dateFormat).replace(/[^a-zA-Z 0-9]+/g,'')).addClass('highlight');
...
Exhibit.DatePickerFacet.DatePicker.prototype.startHighlighting = function(date) {
  ...
  elmt = Exhibit.jQuery('#'+ elmtId).addClass('highlight');
};
